...
=item $map = $node->attributes

If the node is an element, the L<Web::DOM::NamedNodeMap> object
containing the content attributes for the element is returned.
Otherwise, C<undef> is returned.  [DOM] [DOM3CORE] [MANAKAI]
...
Return whether the node has any content attribute or not.  [MANAKAI]
