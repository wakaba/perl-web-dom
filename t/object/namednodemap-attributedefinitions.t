...
sub GET ($$) {
  my ($dt, $name) = @_;
  my $node = $dt->get_attribute_definition_node ($name);
  return $node ? $node->get_user_data ('value') : undef;
} # GET

sub SET ($$$) {
  my ($dt, $name, $value) = @_;
  my $node = $dt->owner_document->create_attribute_definition ($name);
  $node->set_user_data (value => $value);
  $dt->set_attribute_definition_node ($node);
} # SET
...
} n => 5, name => 'attribute_definitions empty';
...
} n => 5, name => 'attribute_definitions not empty';
...
test {
  ...
  my $nl = $node->attribute_definitions;
  my $nl2 = $node->attribute_definitions;
  
  is $nl2, $nl;

  my $node2 = $doc->create_element_type_definition ('b');
  my $nl3 = $node2->attribute_definitions;
...
} n => 4, name => 'attribute_definitions liveness and sameness';
...
test {
  ...
  is scalar @$nl, 0;
  ok not $node->attribute_definitions->length;
  ok not $node2->parent_node;

  done $c;
} n => 6, name => 'attribute_definitions read-only';
...
test {
  ...
  ok not $nl2 eq undef;
  ok $nl2 ne undef;
  ok $nl2 eq $node->attribute_definitions;
  ok not $nl2 ne $node->attribute_definitions;
  is $nl2 cmp $node->attribute_definitions, 0;
  isnt $nl2 cmp $node->attribute_definitions, undef;
...
} n => 13, name => 'attribute_definitions comparison';
...
} n => 22, name => 'attribute_definitions items, empty';
...
test {
  ...
  isa_ok $nl->item (0), 'Web::DOM::AttributeDefinition';
  my $el = $nl->item (0);
...
} n => 37, name => 'attribute_definitions items, empty';
...
} n => 12, name => 'attribute_definitions perl binding empty';
...
} n => 14, name => 'attribute_definitions perl binding not empty';

test {
  my $c = shift;
  my $doc = new Web::DOM::Document;
  my $el = $doc->create_element_type_definition ('a');
  my $col = $el->attribute_definitions;
...
test {
  ...
  isa_ok $attr1, 'Web::DOM::AttributeDefinition';
...
for my $method (qw(set_named_item)) {
  test {
    ...
    my $doc = new Web::DOM::Document;
    my $el = $doc->create_element_type_definition ('a');
    my $attr = $doc->create_attribute_definition ('b');
  ...
  test {
    ...
    is GET ($el, 'b'), '';
    isa_ok $attr2, 'Web::DOM::AttributeDefinition';
    is $attr2->owner_element_type_definition, undef;
  ...
  test {
    ...
    is $attr->node_type, $attr->ATTRIBUTE_DEFINITION_NODE;
    is $attr->owner_element_type_definition, $el;
  ...
  test {
    ...
    my $col = $el2->attribute_definitions;

    dies_here_ok {
      $col->$method ($el->attribute_definitions->[0]);
    };
    isa_ok $@, 'Web::DOM::Exception';
    is $@->name, 'HierarchyRequestError';
    is $@->message, 'The specified node has already attached to another node';
    
    is $el->attribute_definitions->[0]->owner_element_type_definition, $el;
    is GET ($el, 'fuga'), 'abc';
  ...
  test {
    ...
    is $el->attribute_definitions->length, 0;
...
test {
  ...
  isa_ok $node, 'Web::DOM::AttributeDefinition';
...
test {
  ...
  is $@->name, 'NotSupportedError';
  is $@->message, 'This operation is not supported';
  is $el->attribute_definitions->length, 1;
  is $el->attribute_definitions->[0]->owner_element_type_definition, $el;
  is GET ($el, 'hoge'), 'abc';
