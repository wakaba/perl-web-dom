...
sub GET ($$) {
  my ($dt, $name) = @_;
  my $node = $dt->get_general_entity_node ($name);
  return $node ? $node->public_id : undef;
} # GET

sub SET ($$$) {
  my ($dt, $name, $value) = @_;
  my $node = $dt->owner_document->create_general_entity ($name);
...
} n => 5, name => 'general_entities not empty';
...
test {
  ...
  my $nl = $node->general_entities;
  my $nl2 = $node->general_entities;
  
  is $nl2, $nl;

  my $node2 = $doc->create_document_type_definition ('b');
  my $nl3 = $node2->general_entities;
...
} n => 4, name => 'general_entities liveness and sameness';
...
test {
  ...
  is scalar @$nl, 0;
  ok not $node->general_entities->length;
  ok not $node2->parent_node;

  done $c;
} n => 6, name => 'general_entities read-only';

test {
  my $c = shift;
  my $doc = new Web::DOM::Document;
  my $node = $doc->create_document_type_definition ('a');
  my $node2 = $doc->create_general_entity ('a');
  my $node3 = $doc->create_general_entity ('a');
  $node->set_general_entity_node ($node3);
...
} n => 8, name => 'general_entities read-only';
...
test {
  ...
  ok not $nl2 eq undef;
  ok $nl2 ne undef;
  ok $nl2 eq $node->general_entities;
  ok not $nl2 ne $node->general_entities;
  is $nl2 cmp $node->general_entities, 0;
  isnt $nl2 cmp $node->general_entities, undef;
...
} n => 13, name => 'general_entities comparison';
...
} n => 22, name => 'general_entities items, empty';
...
test {
  ...
  isa_ok $nl->item (0), 'Web::DOM::Entity';
  my $el = $nl->item (0);
...
} n => 37, name => 'general_entities items, empty';
...
} n => 12, name => 'general_entities perl binding empty';
...
} n => 14, name => 'general_entities perl binding not empty';

test {
  my $c = shift;
  my $doc = new Web::DOM::Document;
  my $el = $doc->create_document_type_definition ('a');
  my $col = $el->general_entities;
...
for my $method (qw(set_named_item)) {
  test {
    ...
    my $doc = new Web::DOM::Document;
    my $el = $doc->create_document_type_definition ('a');
    my $attr = $doc->create_general_entity ('b');
    my $col = $el->general_entities;
  ...
  test {
    ...
    is $attr->node_type, $attr->ENTITY_NODE;
  ...
  test {
    ...
    my $col = $el2->general_entities;

    dies_here_ok {
      $col->$method ($el->general_entities->[0]);
    };
    isa_ok $@, 'Web::DOM::Exception';
    is $@->name, 'HierarchyRequestError';
    is $@->message, 'The specified node has already attached to another node';
    
    is $el->general_entities->[0]->owner_document_type_definition, $el;
    is GET ($el, 'fuga'), 'abc';
  ...
  test {
    ...
    is $el->general_entities->length, 0;
...
test {
  ...
  is $@->name, 'NotSupportedError';
  is $@->message, 'This operation is not supported';
  is $el->general_entities->length, 1;
  is $el->general_entities->[0]->owner_document_type_definition, $el;
  is GET ($el, 'hoge'), 'abc';
...
test {
  ...
  my $node = $doc->create_document_type_definition ('hoge');
  my $map = $node->general_entities;
  $node->set_general_entity_node
      ($doc->create_general_entity ('hoge'));
  $node->set_general_entity_node
      ($doc->create_general_entity ('hoge:aaa'));
  is $map->{120}, undef;
...
} n => 2, name => '@{} mutation set_general_entity_node';
...
test {
  ...
  $node->remove_general_entity_node
      ($node->get_general_entity_node ('hoge'));
